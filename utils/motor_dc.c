...
#ifndef __LIBMOTORDC__
#define __LIBMOTORDC__
...
// Liga as saidas
  ...
  while((SYSCTL_PRGPIO_R & (SYSCTL_PPGPIO_P4 + SYSCTL_PPGPIO_P5) ) != (SYSCTL_PPGPIO_P4 + SYSCTL_PPGPIO_P5) ){};
	...
	GPIO_PORTF_AHB_DIR_R |= 1<<MOTOR_PWM_PIN;
		
	// 5. Limpar os bits AFSEL para 0 para selecionar GPIO sem função alternativa	
	GPIO_PORTE_AHB_AFSEL_R &= ~(1<<MOTOR_INPUT_A | 1<<MOTOR_INPUT_B);
	GPIO_PORTF_AHB_AFSEL_R &= ~(1<<MOTOR_PWM_PIN);
		
	// 6. Setar os bits de DEN para habilitar I/O digital	
	GPIO_PORTE_AHB_DEN_R |= 1<<MOTOR_INPUT_A | 1<<MOTOR_INPUT_B;
	GPIO_PORTF_AHB_DEN_R |= 1<<MOTOR_PWM_PIN;
	
	
// Configura o timer para o PWM do motor
...
// sign(vel) é a direcao e abs(vel) é o duty cycle do pwm
